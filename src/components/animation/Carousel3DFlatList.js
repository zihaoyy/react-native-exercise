    ...
    <View style={{ backgroundColor: "#A5F1FA", flex: 1 }}>
      <SafeAreaView style={{ marginTop: SPACING * 4 }}>
        ...
        >
          <TouchableOpacity
            disabled={index === 0}
            style={{ opacity: index === 0 ? .2 : 1 }}
            onPress={() => {
              ref?.current?.scrollToOffset({ offset: (index - 1) * width, animated: true });
            }}>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              <Text style={{ fontSize: 12, fontWeight: "800", color: "#222" }}>← PREV</Text>
            </View>
          </TouchableOpacity>
          <TouchableOpacity
            disabled={index === DATA.length - 1}
            style={{ opacity: index === DATA.length - 1 ? .2 : 1 }}
            onPress={() => {
              ref?.current?.scrollToOffset({ offset: (index + 1) * width, animated: true });
            }}>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              <Text style={{ fontSize: 12, fontWeight: "800", color: "#222" }}>NEXT →</Text>
            </View>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
